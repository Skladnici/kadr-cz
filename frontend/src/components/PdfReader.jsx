...
// Zoom range for the +/- controls below. Pages are rendered once, up
// front, at enough native resolution to stay crisp all the way to
// ZOOM_MAX (see renderPage's own comment) — zooming afterward is a plain
...
// open a /podepsat link. 2400px is already sharper than this reader ever
// displays a page at (max ~900 CSS px wide × 2.5 zoom = 2250).
...
export default function PdfReader({ data, className = "", style }) {
  ...
  useEffect(() => {
    ...
    let destroyed = false;
    let pdfDoc = null;
    // The in-flight page.render() task, if any — needed so cleanup can
    // cancel it before destroying the document (see the cleanup's own
    // comment below for why that order matters).
    ...
    const renderPage = async (doc, pageNum) => {
      ...
      // Rendered once at enough resolution to cover the full zoom range
      // (see ZOOM_MAX/MAX_RENDER_WIDTH_PX above) — every zoom step after
      // this is a CSS resize of the same canvas, not a re-render.
      const idealWidth = targetWidth * ZOOM_MAX * (window.devicePixelRatio || 1);
      const renderWidth = Math.min(idealWidth, MAX_RENDER_WIDTH_PX);
  ...
  // native resolution (see renderPage above), so this never re-renders
  // pdf.js output, just how large the existing bitmap is displayed.
  useEffect(() => {
    const container = containerRef.current;
    if (!container || !baseWidthRef.current) return;
    container.querySelectorAll(".pdf-reader-page").forEach((canvas) => {
      canvas.style.width = `${baseWidthRef.current * zoom}px`;
    });
  }, [zoom, rendering]);
  ...
  return (
    <div className={`pdf-reader relative flex flex-col rounded-xl border border-slate-200 bg-slate-100 ${className}`} style={style}>
      {/* Own header row rather than an overlay on top of the pages — no
          ...
          Hidden until rendering finishes since zooming a page that isn't
          drawn yet has nothing to act on. */}
      {!rendering && (
